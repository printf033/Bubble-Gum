...
int Peer_udp::getFd() const { return my_fd_; }
void Peer_udp::send(sockaddr_in &ur_sockaddr_in, const std::string &data)
{
    ::sendto(my_fd_, data.c_str(), data.size() + 1, 0, (const sockaddr *)&ur_sockaddr_in, socklen_);
}
void Peer_udp::send(const std::string &ur_ip, const int ur_port, const std::string &data)
{
    sockaddr_in ur_sockaddr_in{};
    ur_sockaddr_in.sin_family = AF_INET;
...
std::string Peer_udp::recv(sockaddr_in &ur_sockaddr_in)
{
    char buf[MAX_RECV_SIZE]{};
